...
def _diff_enums(old: list, new: list) -> list[Change]:
    changes: list[Change] = []
    old_map, new_map = _by_id(old), _by_id(new)

    for enum_id, entry in new_map.items():
        if enum_id not in old_map:
            changes.append(Change("enum_added", entry["name"], SAFE, "добавлено перечисление", {"enum": entry}))
            continue
        before, after = _by_id(old_map[enum_id]["values"]), _by_id(entry["values"])
        for value_id, value in after.items():
            if value_id not in before:
                changes.append(
                    Change("enum_value_added", f"{entry['name']}.{value['name']}", SAFE,
                           f"добавлено значение {value['name']}", {"enum": entry["name"], "value": value})
                )
            elif before[value_id]["name"] != value["name"]:
                changes.append(
                    Change("enum_value_renamed", f"{entry['name']}.{value_id}", SAFE,
        ...
        for value_id, value in before.items():
            if value_id not in after:
                changes.append(
                    Change("enum_value_removed", f"{entry['name']}.{value['name']}", BREAKING,
                           f"удалено значение {value['name']}", {"enum": entry["name"], "value": value})
                )

    for enum_id, entry in old_map.items():
        if enum_id not in new_map:
            changes.append(
                Change("enum_removed", entry["name"], BREAKING, "удалено перечисление", {"enum": entry})
            )
    return changes


def _diff_types(old: list, new: list) -> list[Change]:
    changes: list[Change] = []
    old_map, new_map = _by_id(old), _by_id(new)
    ...
    for type_id, entry in old_map.items():
        if type_id not in new_map:
            changes.append(
                Change("type_removed", entry["code"], DESTRUCTIVE,
...
def _diff_fields(old_type: dict[str, Any], new_type: dict[str, Any]) -> list[Change]:
    ...
    for field_id, field in after.items():
        ...
        if field_id not in before:
            ...
            changes.append(Change("field_added", element, kind, detail, {"type": code, "field": field}))
            continue

        was = before[field_id]
        if was["name"] != field["name"]:
            changes.append(
                Change("field_renamed", element, SAFE, f"{was['name']} -> {field['name']}",
                       {"type": code, "from": was, "to": field})
            )
        if (was["kind"], was["type"]) != (field["kind"], field["type"]):
            changes.append(
                Change("field_type_changed", element, DESTRUCTIVE,
        ...
        if not was["required"] and field["required"]:
            ...
            changes.append(
                Change("field_required", element, kind,
                       f"поле {field['name']} стало обязательным"
                       + (f" начиная с {field['required_since']}" if field["required_since"] else " без required_since"),
    ...
    for field_id, field in before.items():
        if field_id not in after:
            changes.append(
                Change("field_removed", f"{code}.{field_id}", DESTRUCTIVE,
...
def _diff_relations(old: list, new: list) -> list[Change]:
    changes: list[Change] = []
    old_map, new_map = _by_id(old), _by_id(new)
    ...
    for relation_id, entry in new_map.items():
        if relation_id not in old_map:
            changes.append(
                Change("relation_added", entry["name"], SAFE, "добавлена связь", {"relation": entry})
            )
            continue
        was = old_map[relation_id]
        if was["name"] != entry["name"]:
            changes.append(
                Change("relation_renamed", str(relation_id), SAFE,
                       f"связь {was['name']} -> {entry['name']}", {"from": was, "to": entry})
            )
        for side in ("from", "to"):
            ...
            if added:
                changes.append(
                    Change(f"relation_{side}_widened", entry["name"], SAFE,
                           f"{side}: добавлены типы {', '.join(added)}", {"relation": entry, "codes": added})
                )
            if removed:
                changes.append(
                    Change(f"relation_{side}_narrowed", entry["name"], BREAKING,
                           f"{side}: удалены типы {', '.join(removed)}", {"relation": entry, "codes": removed})
                )
    ...
    for relation_id, entry in old_map.items():
        if relation_id not in new_map:
            changes.append(
                Change("relation_removed", entry["name"], DESTRUCTIVE,
                       "удалена связь, её рёбра будут удалены", {"relation": entry})
            )
    return changes


def _diff_cardinality(name: str, side: str, was: dict, now: dict) -> list[Change]:
    if was == now:
        return []
    ...
    return [
        Change("relation_cardinality", f"{name}.{side}", kind,
